...
import { Key, Plus, Trash2, Edit2, RotateCcw, Check, X, Loader2 } from 'lucide-react';
...
import { serpKeysApi } from '@/lib/apiClient';
...
export default function SerpKeysPage() {
  const { user } = useAuth();
  const [keys, setKeys] = useState<SerpKey[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingKey, setEditingKey] = useState<SerpKey | null>(null);
  ...
  const loadKeys = async () => {
    try {
      const data = await serpKeysApi.list();
      setKeys(data);
    } catch (error: any) {
      toast({
  ...
  const handleOpenDialog = (key?: SerpKey) => {
    if (key) {
      setEditingKey(key);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingKey) {
        await serpKeysApi.update(editingKey.id, {
          name: formName,
  ...
  const handleToggleActive = async (key: SerpKey) => {
    try {
      await serpKeysApi.update(key.id, { isActive: !key.is_active });
      loadKeys();
      toast({ 
  ...
  const handleDelete = async (key: SerpKey) => {
    if (!confirm(`Tem certeza que deseja remover a chave "${key.name}"?`)) {
      return;
    }

    try {
      await serpKeysApi.delete(key.id);
      loadKeys();
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight neon-text-cyan">Chaves SERP API</h1>
          <p className="text-muted-foreground">Gerencie múltiplas chaves com rotação automática</p>
        </div>
        <div className="flex gap-2">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{editingKey ? 'Editar Chave' : 'Nova Chave SERP'}</DialogTitle>
                <DialogDescription>
                  {editingKey 
                    ? 'Atualize as informações da chave. Deixe a API Key em branco para manter a atual.'
                    : 'Adicione uma nova chave da SERP API para o sistema usar nas pesquisas.'}
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  ...
                  <Input
                    id="name"
                    placeholder="Ex: Chave Principal, Backup 1..."
                    value={formName}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="apiKey"
                    type="password"
                    placeholder="Sua chave da SERP API"
                    value={formApiKey}
