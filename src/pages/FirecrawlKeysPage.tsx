...
import { Key, Plus, Trash2, Edit2, RotateCcw, Loader2, Flame } from 'lucide-react';
...
import { firecrawlKeysApi } from '@/lib/apiClient';
...
export default function FirecrawlKeysPage() {
  const { user } = useAuth();
  const [keys, setKeys] = useState<FirecrawlKey[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingKey, setEditingKey] = useState<FirecrawlKey | null>(null);
  
  // Form state
  const [formName, setFormName] = useState('');
  const [formApiKey, setFormApiKey] = useState('');
  const [formMonthlyLimit, setFormMonthlyLimit] = useState('500');
  ...
  const loadKeys = async () => {
    try {
      const data = await firecrawlKeysApi.list();
      setKeys(data);
    } catch (error: any) {
      toast({
  ...
  const handleOpenDialog = (key?: FirecrawlKey) => {
    if (key) {
      setEditingKey(key);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingKey) {
        await firecrawlKeysApi.update(editingKey.id, {
          name: formName,
  ...
  const handleToggleActive = async (key: FirecrawlKey) => {
    try {
      await firecrawlKeysApi.update(key.id, { isActive: !key.is_active });
      loadKeys();
      toast({ 
  ...
  const handleDelete = async (key: FirecrawlKey) => {
    if (!confirm(`Tem certeza que deseja remover a chave "${key.name}"?`)) {
      return;
    }

    try {
      await firecrawlKeysApi.delete(key.id);
      loadKeys();
  ...
  const handleResetUsage = async () => {
    ...
    try {
      await firecrawlKeysApi.resetUsage();
      loadKeys();
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight neon-text-cyan flex items-center gap-2">
            ...
            Chaves Firecrawl (Instagram)
          </h1>
          <p className="text-muted-foreground">Gerencie múltiplas chaves com rotação automática para scraping do Instagram</p>
        </div>
        <div className="flex gap-2">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{editingKey ? 'Editar Chave' : 'Nova Chave Firecrawl'}</DialogTitle>
                <DialogDescription>
                  {editingKey 
                    ? 'Atualize as informações da chave. Deixe a API Key em branco para manter a atual.'
                    : 'Adicione uma nova chave do Firecrawl para pesquisas no Instagram.'}
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4 py-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="apiKey">
                    API Key {editingKey && '(deixe em branco para manter)'}
                  </Label>
                  <Input
                    id="apiKey"
                    type="password"
                    placeholder="Sua chave do Firecrawl (fc-...)"
                    value={formApiKey}
                    onChange={(e) => setFormApiKey(e.target.value)}
                  />
                  <p className="text-xs text-muted-foreground">
                    Encontre sua chave em: firecrawl.dev → Dashboard → API Keys
                  </p>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="limit">Limite Mensal de Créditos</Label>
                  <Input
                    id="limit"
                    type="number"
                    placeholder="500"
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Plano Free do Firecrawl: 500 créditos/mês (1 crédito = 1 página)
                  </p>
                </div>
              </div>
              <DialogFooter>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="neon-border">
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              créditos restantes para scraping
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p>Nenhuma chave Firecrawl cadastrada</p>
              <p className="text-sm">Clique em "Nova Chave" para adicionar</p>
              <p className="text-xs mt-2">Crie uma conta gratuita em firecrawl.dev</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
