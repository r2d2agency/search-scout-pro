...
// Listar todas as chaves Apify (superadmin only)
router.get('/', authenticate, requireSuperAdmin, async (req, res) => {
  try {
    ...
    console.error('Erro ao listar chaves Apify:', error);
    res.status(500).json({ message: 'Erro ao buscar chaves Apify' });
  }
});

// Adicionar nova chave Apify (superadmin only)
router.post('/', authenticate, requireSuperAdmin, async (req, res) => {
  try {
    ...
    const result = await db.query(
      `INSERT INTO apify_api_keys (name, api_key, monthly_limit)
       VALUES ($1, $2, $3)
    ...
    console.error('Erro ao adicionar chave Apify:', error);
    res.status(500).json({ message: 'Erro ao adicionar chave Apify' });
  }
});

// Atualizar chave Apify (superadmin only)
router.put('/:id', authenticate, requireSuperAdmin, async (req, res) => {
  try {
    ...
    params.push(id);
    const query = `UPDATE apify_api_keys SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING id, name, is_active, usage_count, monthly_limit, created_at`;
    ...
    console.error('Erro ao atualizar chave Apify:', error);
    res.status(500).json({ message: 'Erro ao atualizar chave Apify' });
  }
});

// Deletar chave Apify (superadmin only)
router.delete('/:id', authenticate, requireSuperAdmin, async (req, res) => {
  try {
    const { id } = req.params;

    const result = await db.query(
      'DELETE FROM apify_api_keys WHERE id = $1 RETURNING id',
      [id]
    );
    ...
    console.error('Erro ao deletar chave Apify:', error);
    res.status(500).json({ message: 'Erro ao remover chave Apify' });
  }
});
...
router.post('/reset-usage', authenticate, requireSuperAdmin, async (req, res) => {
  try {
    await db.query('UPDATE apify_api_keys SET usage_count = 0');
  ...
  try {
    ...
    if (result.rows.length === 0) {
      ...
      if (process.env.APIFY_API_KEY) {
        return { key: process.env.APIFY_API_KEY, source: 'env' };
      }
      console.log('Nenhuma chave Apify disponível');
    ...
    await db.query(
      'UPDATE apify_api_keys SET usage_count = usage_count + 1, last_used_at = NOW() WHERE id = $1',
      [keyData.id]
    );

    return { key: keyData.api_key?.trim(), source: 'global' };
  } catch (error) {
    console.error('Erro ao obter chave Apify:', error);
